...
log "正在创建本地目录结构..."
mkdir -p /config/shell /config/packages /config/www/img /config/themes
success "目录准备就绪。"
...
curl -sSL --connect-timeout 20 -o /config/themes/mushroom-glass.yaml "${RAW_URL}/themes/mushroom-glass.yaml?v=$T" || { error "下载失败"; exit 1; }
